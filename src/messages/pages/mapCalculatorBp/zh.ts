...
const measurementErrors = [
  ...
  { error: '背部无支撑', bpEffect: '舒张压偏高', mapEffect: 'MAP 升高 2-5 mmHg' },
  { error: '测量时说话', bpEffect: '读数偏高', mapEffect: 'MAP 升高 5-10 mmHg' },
  { error: '膀胱充盈', bpEffect: '+10-15 mmHg', mapEffect: 'MAP 升高 5-7 mmHg' },
  { error: '刚摄入咖啡因 / 烟草', bpEffect: '读数偏高', mapEffect: 'MAP 升高 5-8 mmHg' },
] as const;
...
const scenarios = [
  ...
  {
    heading: '术后监测',
    scenario: '冠脉搭桥术后患者，基线 MAP 85 mmHg；当前血压 95/60 mmHg → MAP 72 mmHg。',
    points: [
      'MAP 较基线下降约 15%。',
      '排查出血、低血容量或镇痛不足。',
      '在升级处理前评估胸管引流与实验室指标。',
...
const trendingExamples = [
  {
    title: '趋势改善',
    timeline: ['第 0 小时：80/50 → MAP 60 mmHg', '第 2 小时：90/55 → MAP 67 mmHg', '第 4 小时：100/60 → MAP 73 mmHg'],
    summary: '对治疗反应良好。',
  },
  {
    title: '趋势恶化',
    timeline: ['第 0 小时：110/70 → MAP 83 mmHg', '第 2 小时：100/65 → MAP 77 mmHg', '第 4 小时：90/55 → MAP 67 mmHg'],
    summary: '立即查找潜在原因。',
  },
] as const;
...
const mapCalculatorBpZh: MapCalculatorBpContent = {
  ...
  hero: {
    ...
    description: '输入常规血压值即可即时获取平均动脉压，并附带临床意义提示，适合在 ICU、急诊及围术期护理中迅速评估灌注状况。',
  ...
  sections: {
    whyCalculate: {
      ...
      clinicalReality: {
        heading: '临床现实',
        intro: '大多数自动监护仪不会默认显示 MAP，但几乎所有重症路径都依赖它。在医院的各类场景中，MAP 用于：',
        items: [
          { label: '脓毒症流程：', body: '维持 MAP ≥65 mmHg 以达到《生存脓毒症运动》目标。' },
          { label: 'ICU 监护：', body: '在休克、术后及依赖血管加压药的患者中监测器官灌注趋势。' },
          { label: '血管加压药滴定：', body: '根据 MAP 目标调节去甲肾上腺素、加压素或去氧肾上腺素。' },
          { label: '术后恢复：', body: '在症状出现前捕捉灌注下降的早期信号。' },
        ],
      },
      usage: {
        heading: '何时需要这个工具',
        intro: 'MAP 计算把记录的生命体征与流程指标连接起来。需要在第一时间搞清楚：',
        questions: ['当前灌注压是否满足患者状况？', '是否需要补液、使用升压药或降压药？', '过去数小时的 MAP 趋势如何？', '是否需要调整输注速度或升级治疗层级？'],
        outro: '由于无创设备无法直接测量 MAP，掌握该计算是所有照护危重患者的临床人员必须具备的能力。',
      },
    },
    bpToMap: {
      heading: '血压如何换算成 MAP',
      ...
      simpleAverageIntro: '简单平均会忽略心动周期固有的时间权重。例如血压 120/80 mmHg 时：',
      simpleAveragePoints: ['简单平均： (120 + 80) ÷ 2 = 100 mmHg', '加权 MAP 公式： (120 + 160) ÷ 3 = 93 mmHg'],
      simpleAverageConclusion: '相差 7 mmHg 在临床上具有重要意义，尤其在低血压患者中，差异可能决定灌注是否足够或需要升级治疗。',
      alternative: {
        heading: '有创监测下的替代公式',
        intro: '动脉置管会直接显示 MAP，但临床人员常用心算进行验证：',
        formula: 'MAP = 舒张压 +（收缩压 − 舒张压）÷ 3',
        explanation: '该变式使用脉压进行换算，做床旁心算时更快捷。两个公式在代数上等价，结果完全一致。',
      },
      accuracy: {
        heading: '准确性注意事项',
        intro: '无创血压推算的 MAP 对多数成人患者可靠，但在收缩期与舒张期时程被改变的情境下需谨慎：',
        items: ['重度主动脉瓣返流导致脉压过宽', '极度心动过速缩短舒张期', '伴快速心室反应的房颤', '晚期外周血管疾病或血管不可压'],
        note: '若出现上述情况，应尽可能优先选择动脉有创监测。',
      },
    },
    referenceGuide: {
      heading: '血压换算 MAP 参考指南',
      intro: '使用下列速查表即可在床旁快速解读结果。数据与常见医院流程保持一致，并提示何时需要升级处理。',
      mapTableHeaders: {
        bp: '血压读数',
        formula: 'MAP 计算式',
        map: 'MAP 结果',
      ...
      pulsePressureIntro: 'MAP 趋势需要结合脉压（收缩压 − 舒张压）一同分析。即使 MAP 相同，脉压宽度不同也可能代表截然不同的血流动力学状况。',
    ...
    measurement: {
      ...
      intro: 'MAP 的准确性取决于血压测量质量。使用这份检查清单，避免可以预防的测量误差。',
      preparation: {
        heading: '患者准备（5 分钟）',
        steps: ['让患者坐姿端正，背部有支撑，双脚不交叉。', '将上肢置于心脏同一水平，可借助桌面或枕垫。', '如膀胱胀满，先让患者排空。', '测量前 30 分钟内避免咖啡因、运动或吸烟。', '测量前静坐休息 5 分钟。', '测量过程中保持安静，不交谈、不玩手机。'],
      },
      cuff: {
        heading: '袖带选择与放置',
        bullets: ['选择袖带囊宽约为臂围 40% 的型号。', '袖带下缘距肘窝 2-3 厘米。', '使气管与肱动脉走向一致。', '松紧适度，可容纳两指但不感疼痛。'],
        processHeading: '测量步骤',
        steps: ['加压同时触诊桡动脉估算收缩压。', '在脉搏消失点基础上再加压 20-30 mmHg。', '以每秒 2-3 mmHg 的速度缓慢放气。', '听到第一相柯氏音时记录收缩压。', '声音完全消失时记录舒张压。', '间隔 1-2 分钟重复测量，取平均值。'],
      },
      ...
      whenInaccurate: {
        ...
        bullets: ['MAP <50 mmHg 或重度依赖升压药的休克', '重度肥胖且无法获得合适袖带', '严重心律失常（如房颤合并快速心室率）', '需逐搏监测的高风险术中病例', '重度外周血管疾病或血管不可压'],
        note: '在这些情况下，如需确保精确，请升级为动脉有创监测。',
      },
    },
    decisionMaking: {
      heading: '将 MAP 换算结果用于临床决策',
      intro: '只有结合临床背景，MAP 数值才具有实际意义。以下场景可帮助你把数字转化为行动。',
      scenarioLabel: '情境：',
      scenarios,
      trending: {
        heading: '趋势比单次读数更重要',
        intro: '记录 MAP 趋势而非单一读数。数小时内的改善或恶化比孤立数据更能反映灌注状况。',
